    ...
    <div class="flex items-center gap-3 mb-5">
      <div class="relative flex-1 max-w-xs">
        ...
        <input [value]="search()" (input)="search.set($any($event.target).value)"
               class="form-input pl-9" placeholder="Buscar por cliente, DNI, matrícula...">
      </div>
...
export class RentalListComponent implements OnInit {
  ...
  filtered = computed(() => {
    ...
    return this.rentals().filter(r => {
      const matchSearch = !q || r.client.name.toLowerCase().includes(q) || r.client.dni.toLowerCase().includes(q) || r.car.licensePlate.toLowerCase().includes(q);
